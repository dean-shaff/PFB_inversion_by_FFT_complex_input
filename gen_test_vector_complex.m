function gen_test_vector_complex(Wave_type,impulse_offset,impulse_width,Nout,Nblocks,f_sample_out,period,noise,fname)
...
%
% I. Morrison      24-Oct-2016  Complex output, like LFAA coarse channels
% 
% ----------------------------------------------------------------------
...
Nin = Nout; % Number of data elements in input time series
Pmul = 1.0; % Power multiplication factor for all but the DC channel
...
for ii = 1:Nblocks,
    ...
    if(Wave_type == 1)  % Impulse case
        ...
        z1 = complex(zeros(Nout,1,'single'));
    ...
    z = [real(z1), imag(z1)];
    dat = reshape(transpose(z),2*Nout,1);
